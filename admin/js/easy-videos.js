jQuery(function ($) {
    ...
    // Select / unselect single video
    $(document).on('change', '.easy-video-checkbox-item', function () {
        let id = $(this).attr('data-id'),
            name = $(this).attr('data-name')
        if ($(this).is(':checked')) {
            $(this).parent().append('<input type="hidden" name="videos[' + id + '][title]" value="' + name + '">');
        } else {
    ...
    // Select all
    $(document).on('change', '#easy-videos-select-all', function () {
        $('#easy-videos-import .easy-video-checkbox-item').prop("checked", $(this).is(':checked')).change()
    });

    // Preview video
    $(document).on('click', '.easy-videos-preview', function () {
        $(this).parents('li').find('.easy-video-iframe, .easy-video-image').toggle()
    });
});
...
function addPlaceholder(wrapper) {
    ...
    if (!tokens.length && !(document.activeElement === input_search))
        input_search.setAttribute("placeholder", "Category");
}
...
function createInitialTokens(select) {
    ...
    for (let i = 0; i < options_selected.length; i++) {
        createToken(wrapper, options_selected[i].title, e.target.dataset.title);
    }
}
...
function createToken(wrapper, value, id) {
    ...
    close.setAttribute("data-value", id);
...
function populateAutocompleteList(select, query, dropdown = false) {
    ...
    if (result_size == 1) {
        ...
        li.innerText = options_to_show[0].title;
        li.setAttribute('data-value', options_to_show[0].value);
        li.setAttribute('data-title', options_to_show[0].title);
        ...
        for (let i = 0; i < result_size; i++) {
            const li = document.createElement("li");
            li.innerText = options_to_show[i].title;
            li.setAttribute('data-value', options_to_show[i].value);
            li.setAttribute('data-title', options_to_show[i].title);
            li.addEventListener("click", selectOption);
            autocomplete_list.appendChild(li);
        }
    } else {
...
function selectOption(e) {
    ...
    createToken(wrapper, e.target.dataset.title, e.target.dataset.value);
    if (input_search.value) {
        input_search.value = "";
    }
...
function removeToken(e) {
    ...
    const value_to_remove = e.target.dataset.value;
    const wrapper = e.target.parentNode.parentNode;
    const input_search = wrapper.querySelector(".selected-input");
    const dropdown = wrapper.querySelector(".dropdown-icon");
